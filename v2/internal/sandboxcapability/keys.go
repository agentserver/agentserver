...
func LoadSigner(issuer, audience, keyID, filePath string) (*Signer, error) {
	raw, err := readStableFile("sandbox capability signing key", filePath, maximumPrivateKeyBytes, true)
	if err != nil {
		return nil, err
	}
	defer clear(raw)
	...
	return NewSigner(issuer, audience, keyID, privateKey)
}

func LoadVerifier(filePath string) (*Verifier, error) {
	raw, err := readStableFile("sandbox capability verification keyring", filePath, maximumKeyringBytes, false)
	if err != nil {
		return nil, err
	}
...
func ParseVerifier(raw []byte) (*Verifier, error) {
	if len(raw) < 1 || len(raw) > maximumKeyringBytes {
		return nil, fmt.Errorf("sandbox capability keyring must contain between 1 and %d bytes", maximumKeyringBytes)
	}
	...
	if _, _, err := braincatalog.DecodeCanonicalJSON(raw, maximumKeyringBytes, limits); err != nil {
		return nil, fmt.Errorf("validate sandbox capability keyring JSON: %w", err)
	}
	decoder := json.NewDecoder(bytes.NewReader(raw))
	decoder.DisallowUnknownFields()
	var document KeyringDocument
	if err := decoder.Decode(&document); err != nil {
		return nil, fmt.Errorf("decode sandbox capability keyring: %w", err)
	}
	if err := finishJSON(decoder); err != nil {
		return nil, fmt.Errorf("finish sandbox capability keyring: %w", err)
	}
	if document.Version != KeyringVersion || len(document.Keys) < 1 || len(document.Keys) > 64 {
		return nil, errors.New("sandbox capability keyring version or key count is invalid")
	}
	...
	for index, entry := range document.Keys {
		...
		if err != nil || len(publicKey) != ed25519.PublicKeySize || base64.RawURLEncoding.EncodeToString(publicKey) != entry.PublicKey {
			return nil, fmt.Errorf("keys[%d].publicKey must be canonical base64url for a 32-byte Ed25519 key", index)
		}
		keys = append(keys, TrustedKey{
			Issuer: entry.Issuer, Audience: entry.Audience, KeyID: entry.KeyID,
...
func decodePrivateKey(raw []byte) (ed25519.PrivateKey, error) {
	...
	if block == nil || block.Type != "PRIVATE KEY" || len(block.Headers) != 0 || len(bytes.TrimSpace(rest)) != 0 {
		return nil, errors.New("sandbox capability signing key must be a raw Ed25519 seed/private key or one unencrypted PKCS#8 PRIVATE KEY PEM block")
	}
	defer clear(block.Bytes)
	parsed, err := x509.ParsePKCS8PrivateKey(block.Bytes)
	if err != nil {
		return nil, fmt.Errorf("parse sandbox capability PKCS#8 private key: %w", err)
	}
	privateKey, ok := parsed.(ed25519.PrivateKey)
	if !ok {
		return nil, errors.New("sandbox capability PKCS#8 private key is not Ed25519")
	}
