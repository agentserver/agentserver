...
独立agentx提交`066acf6`已经实现上述边界。最新Linux arm64原生门禁在无网络、只读rootfs、root connector仅有`CHOWN/SETUID/SETGID`、runner UID/GID 65532且零capability的Apple Container VM中通过：root/target替换、内外symlink与FIFO resolver门禁20轮，fd 3唯一继承20轮，connector→runner identity/PDEATHSIG门禁20轮，官方stock Codex 0.146.0完整`fs/open → fs/readBlock → fs/close → cleanup`链5轮。随后`b4fb3eb`的cgroup事务/`setsid`回收、runner停止后的no-`CAP_KILL` fallback、runner/guardian/connector SIGKILL和同一stock safe-open+cgroup链又各连续通过20轮。该证据关闭Linux filesystem safe-open与process-tree containment，但不解决已校验runtime executable在mutable安装路径上被替换的safe-exec/immutable-install边界。
...
该状态仍不能称为生产可部署executor，但身份链、filesystem safe-open与Linux process-tree containment已经闭合。Core的双钥enrollment、Hydra v26.2.0 ES256 `private_key_jwt`、opaque短期token在线授权/吊销已经实现；llmproxy和executor-gateway也已有production serve、本地Ed25519 run-capability验签及逐请求Core live-authorize。executor-gateway额外完成deployment-bound enrollment relay、256-bit/30秒进程内challenge、Ed25519 WSS proof、replay/expiry/Core故障/revoke门禁、TLS/mTLS SPIFFE装配和真实进程级production E2E；`serve --insecure-dev`只保留为显式开发入口。独立agentx的`548619a`身份/连接切片已实现stdin-only一次性enrollment、owner-only Ed25519/P-256双私钥与模糊提交恢复、detached-signed runtime release、RFC 7523 ES256换token、每次物理fresh/resume WSS的新challenge签名，以及connector→降权runner/exec-server的credential/FD/env隔离；`066acf6`以Linux `openat2`和只读继承descriptor关闭production filesystem path TOCTOU，`b4fb3eb`又以CAP_CHOWN-only guardian、原子cgroup placement、迁移冻结事务和全退出`cgroup.kill`关闭`setsid`/double-fork逃逸。仍未完成的是gateway进程丢失后的dispatching/acknowledged恢复审计、正式签名runtime release的immutable/safe-exec安装边界，以及生产Kubernetes/IAM/Secret/NetworkPolicy清单与故障注入。任何未关联business RPC与不匹配MCP call metadata仍会fail closed。

进入实现前必须完成以下 Phase 0 gate：

- [ ] 固定Codex版本与Codex/外部executable digest，完成stock exec-server stdio启动/RPC fixture、outer capability排除`process/signal`、每process独占instance、正常关闭与root/descendant异常时无旁路process-tree回收、agentx代理兼容测试（reference adapter、真实agentx纵向兼容、Linux arm64 filesystem safe-open和cgroup v2 process-tree门禁已在exact stable 0.146.0通过；正式签名runtime lock的immutable/safe-exec安装仍待完成，非Linux平台仍不得标production）。
