...
func validateCompleteOperation(command CompleteOperationCommand) error {
	...
	if !isTerminalOperationStatus(command.TerminalStatus) {
...
func aggregateExecutionStatus(executionStatus string, statuses []string) (string, error) {
	...
	for _, status := range statuses {
		counts[status]++
		if status == OperationStatusDispatching || status == OperationStatusAcknowledged {
	...
	if counts[OperationStatusSucceeded] == len(statuses) {
		return ExecutionStatusSucceeded, nil
	}
	if executionStatus == ExecutionStatusCancelling && counts[OperationStatusPrepared] > 0 {
		return ExecutionStatusCancelled, nil
	}
	return "", errors.New("execution still has prepared operations without a terminal dispatched outcome")
}
