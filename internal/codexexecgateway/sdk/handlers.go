...
// recordCall emits a single CallStart so the SDK call is captured at
// handler-level granularity. The matching per-WS-frame audit hooks are
// suppressed for sdk-pool-managed bridges in
// codexexecgateway.handleBridge (see captoken.go's SkipAudit field) to
// avoid double-recording the same call.
//
...
//
// Returns ok=true when the call was recorded, ok=false when CallStart
// failed (handler should short-circuit; recordCall has already written
// a 503).
func recordCall(
	rec audit.Recorder,
...
func (s *Server) handleToolCall(w http.ResponseWriter, r *http.Request) {
	...
	if !recordCall(s.Recorder, w, wsID, userID, "", "tool.call:"+req.Tool, bodyBytes) {
		return
	}
...
func (s *Server) handleEnvsList(w http.ResponseWriter, r *http.Request) {
	wsID := workspaceFromCtx(r.Context())
	userID := userIDFromCtx(r.Context())
	if !recordCall(s.Recorder, w, wsID, userID, "", "envs.list", nil) {
		return
	}
...
func (s *Server) handleStdin(w http.ResponseWriter, r *http.Request) {
	...
	if !recordCall(s.Recorder, w, wsID, userID, "", "processes.stdin", bodyBytes) {
		return
	}
...
func (s *Server) handleOutput(w http.ResponseWriter, r *http.Request) {
	...
	wsID := workspaceFromCtx(r.Context())
	userID := userIDFromCtx(r.Context())
	if !recordCall(s.Recorder, w, wsID, userID, "", "processes.output", nil) {
		return
	}
...
func (s *Server) handleTerminate(w http.ResponseWriter, r *http.Request) {
	...
	wsID := workspaceFromCtx(r.Context())
	userID := userIDFromCtx(r.Context())
	if !recordCall(s.Recorder, w, wsID, userID, "", "processes.terminate", nil) {
		return
	}
