...
func TestCoreRunBackendCreatesRunAndReadsScopedCommittedEvents(t *testing.T) {
	...
	client := &http.Client{Transport: browserRoundTripFunc(func(request *http.Request) (*http.Response, error) {
		...
		case 1:
			...
			if err := json.NewDecoder(request.Body).Decode(&body); err != nil || body.Prompt != "hello" || body.ClientRunID != "client-1" || body.ExpectedPermissionModeVersion != 3 || body.ExpectedWorkingDirectoryVersion != 7 {
	...
	started, err := backend.StartRun(t.Context(), StartRunRequest{
		...
		IdempotencyKey: "request-1", ClientRunID: "client-1", Prompt: "hello", ExpectedPermissionModeVersion: 3, ExpectedWorkingDirectoryVersion: 7,
	})
