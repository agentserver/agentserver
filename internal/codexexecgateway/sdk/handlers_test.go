...
// capRec captures CallStart for assertion in recordCall tests.
type capRec struct {
	mu         sync.Mutex
	startErr   error
...
// TestEnvsList_RecorderObservesCallStart: wiring test. Pins that
// handleEnvsList actually invokes the Recorder so a future refactor
// that drops s.Recorder won't silently disable audit for the entire
// SDK surface — at least one handler now fails fast.
func TestEnvsList_RecorderObservesCallStart(t *testing.T) {
