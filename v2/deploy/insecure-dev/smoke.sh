...
case "${checkpoint_count}" in ''|*[!0-9]*) checkpoint_count=0 ;; esac
[ "${checkpoint_count}" -ge 1 ] || { printf '%s\n' 'smoke.sh: no durable checkpoint was committed' >&2; exit 1; }

printf '%s\n' "smoke.sh: passed AG-UI -> Core -> harness-worker -> stock app-server -> MCP -> agentx -> stock exec-server"
printf '%s\n' "smoke.sh: durable checkpoints in this development authority: ${checkpoint_count}"
