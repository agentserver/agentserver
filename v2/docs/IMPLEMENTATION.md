...
这仍不是可生产部署的executor：agentx enrollment/OAuth key binding和core签发的短期run capability尚未实现，`executor-gateway`命令因此仍只暴露显式loopback `serve --insecure-dev`，生产serve模式不存在；MCP principal/run context、TransitionRecord allocator、shell-v1 mapper、完整operation ACK/terminal/unknown收口与MCP shell orchestration仍待下一段。当前MCP只能列环境，尚未对外暴露shell，不能把“`list_environments`可调用”描述成executor执行链已经完成。
