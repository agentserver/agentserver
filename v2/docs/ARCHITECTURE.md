...
- exec-server 自身崩溃时不能假定孙进程随之退出。Linux production agentx把runner放入唯一cgroup v2根，并把每个exec-server/后代原子放入独立子cgroup；任何terminal都先执行`cgroup.kill`再核对`populated=0`和删除。其他平台必须提供等价job/process-tree边界，否则只能标development；在清空确认前execution为`unknown`。
...
- production session 默认 Codex `permissionMode = "read-only"`，用户可通过独立 CAS API 随时修改下一轮为 `read-only | auto | full-access`。Core 在创建 run 的事务中锁住 session 并冻结当时的 mode/version；active run 不受后续切换影响，pool 只能把这份 immutable authority 写入签名 manifest。worker 再把三个 Codex 内置 preset ID 机械映射为原生 `approvalPolicy` / `approvalsReviewer` / `sandbox` 字段，AG-UI prompt、模型和普通 worker wire 输入不能改变它；executor-gateway 同时读取同一签名 capability 中的 mode，按相同 authority 决定产品层是否需要 approval（`read-only` 继续 ask，`auto`/`full-access` 对 bounded executor tool allow，显式部署 `deny` 仍优先）。因此用户切换的是一份跨 Codex 与 AgentServer 的 permission authority，而不是两套独立开关。文件系统、网络、身份、Core live-authority 和 capability 校验不因 allow 而关闭。deployment profile 只为没有显式 Core authority 的 launch source 提供 fallback；旧 manifest 缺少该字段时保留历史的 `approvalPolicy: never` + `read-only` 严格投影。executor 产品审批仍发生在 worker 作为 MCP client 收到的 `elicitation/create` 上，但只在联动后的 policy 仍为 `ask` 时发生，不经过 app-server；
- session 的 executor workspace binding 同样是下一次 Run 的 authority：Core 只持久化 environment UUID、相对 `workingDirectory` 和独立版本，不持久化宿主绝对路径。创建 Run 时在锁住 session 的同一事务中解析当前注册 environment，冻结 environment version、root descriptor digest、相对目录和目录版本；签名 manifest、audience-separated executor capability、MCP principal 与每次 environment lookup 必须逐层保持这份值。目录更新可以在 active Run 期间发生，但旧 Run 的 frozen binding 不变；发起新 Run 的 AG-UI 请求可带 `expectedWorkingDirectoryVersion`，避免 UI 快照与实际执行目录发生静默错位；
- workspace-backed 的代码读写保持手脑分离：worker 的 app-server cwd 是隔离的 attempt 目录，`executor.shell` 才是命令/写入通道，`executor.read_file` 只提供有界读取。gateway 将 shell 默认 cwd 投影为 frozen directory、将 read-file 相对路径自动前缀到该目录，并在 AgentX backend 前验证 environment generation、root digest、路径 containment 和 permission access；当前 TAE 只保留固定 managed-CLI profile，不接受 session workspace binding，直到其 runtime 具备经过门禁的 filesystem access enforcement。skill discovery 由受控 developer instructions 指向固定的 `skills`、`.agents/skills`、`.codex/skills`、`.dsh/skills` roots 和精确 `SKILL.md`；stock Codex 原生 skills/MCP resources 仍保持关闭，项目 skill 内容不具有更高优先级；
...
- skill/context 本身只是只读的不可信项目说明，不能扩大 tool catalog、endpoint、credential 或 permission authority。skill 引用脚本不等于执行授权：只有任务确实需要且当前 Run 允许时，模型才能通过 frozen `executor.shell` 明确执行工作目录内的脚本，并继续接受 backend sandbox/policy 审批；skill 请求注入 stdio MCP、plugin、hook 或另一条执行通道必须拒绝。
...
Linux production实现固定为agentx提交`b4fb3eb`的cgroup v2 guardian协议。root connector只允许exact effective/permitted `CAP_CHOWN|CAP_SETUID|CAP_SETGID`并继承`no_new_privs`；guardian经过二阶段self re-exec后逐runtime thread核对只剩`CAP_CHOWN`，runner/launcher/stock均为独立非root UID/GID、零capability。每次`Prepare`先冻结全部active子cgroup，再临时只给parent/新child的`cgroup.procs`迁移权限；runner以`CLONE_INTO_CGROUP`启动blocked launcher，guardian精确核对PID/PPID/UID/GID/cgroup path后撤权、解冻并Commit，launcher才exec stock。mismatch、guardian错误或连接EOF全部fail closed。生产容器还必须有真正的PID 1 child reaper（例如pinned `tini`）；cgroup能杀死和确认non-populating进程树，但不能代替PID 1回收orphan zombie。
...
独立agentx提交`5d40b6b`已经关闭production Linux的runtime executable safe-open/execute边界。manifest artifact通过`openat2(RESOLVE_BENEATH|RESOLVE_NO_MAGICLINKS|RESOLVE_NO_SYMLINKS|RESOLVE_NO_XDEV)`取得descriptor并由同一descriptor完成hash/size校验；launch plan保留精确Codex inode，blocked launcher只有在cgroup `Commit`后才执行`/proc/self/fd/N`，且该fd在stock exec时为`CLOEXEC`。从`/`到runtime root、Codex和bundled bwrap的完整路径必须全部由UID/GID `0/0`持有、不可group/other写、无symlink/setuid/setgid；因此stock后续按安装路径寻找bwrap仍只能进入受信树，并继续执行0.146.0内置digest校验。`make linux-runtime-live`已在无网络、只读rootfs、真实PID 1 reaper且root connector只有`CHOWN/SETUID/SETGID`的native Linux arm64环境中连续20轮替换Codex、rename runtime root并将旧路径换成symlink、替换bwrap：Codex/root攻击始终执行固定inode，bwrap攻击精确失败为`bundled bubblewrap digest mismatch`，所有poison marker均未执行。该结论只覆盖满足上述安装合同的production Linux；其他平台和新的release/architecture必须重跑对应门禁。
...
stable 0.146.0已实测拒绝携带platform sandbox的流式读取，错误为`streaming file reads do not support platform sandboxing`。因此agentx为每个outer block read启动不允许`process/start`的一次性fs-only stock instance，在本地授权后执行恰好一次`fs/open(sandbox=null) → fs/readBlock → fs/close`，随后关闭该instance并验证containment cleanup；任一步失败都不得遗留可复用handle。`sandbox=null`只是已验证stock限制下的内部调用形状，不是授权依据。显式`insecure-dev`仍在调用前、返回后按registered root重新canonicalize并复核路径；这个同UID开发fallback无法消除symlink TOCTOU，不能据此声明filesystem profile可生产部署。

production Linux runner在开始服务前固定registered workspace root：以`O_PATH|O_DIRECTORY|O_NOFOLLOW`打开并复核配置路径前后与descriptor identity，再实际探测`openat2`和`/proc/self/fd`；任一能力不可用即不发布production filesystem capability。每次读取只把caller URI词法转换为该固定root下的relative path，使用`openat2(RESOLVE_BENEATH|RESOLVE_NO_MAGICLINKS|RESOLVE_NO_SYMLINKS)`先取得`O_PATH` descriptor。先`fstat`确认regular file可避免FIFO阻塞和device副作用，再经`/proc/self/fd/<pinned>`重新取得`O_RDONLY` descriptor并复核两者是同一inode。一次性stock child只继承这一枚descriptor为fd 3，收到的固定路径是`file:///proc/self/fd/3`，从未收到caller path；root或target在授权后rename/replace都不会改变本次读取对象，所有symlink component直接拒绝。read/close/stdio cleanup结束后再次复核固定descriptor并关闭它，任何不确定结果都会让lane fail closed。

独立agentx提交`066acf6`已经实现上述filesystem边界。最新Linux arm64原生门禁在无网络、只读rootfs、root connector仅有`CHOWN/SETUID/SETGID`、runner UID/GID 65532且零capability的Apple Container VM中通过：root/target替换、内外symlink与FIFO resolver门禁20轮，fd 3唯一继承20轮，connector→runner identity/PDEATHSIG门禁20轮，官方stock Codex 0.146.0完整`fs/open → fs/readBlock → fs/close → cleanup`链5轮。随后`b4fb3eb`的cgroup事务/`setsid`回收、runner停止后的no-`CAP_KILL` fallback、runner/guardian/connector SIGKILL和同一stock safe-open+cgroup链又各连续通过20轮；`5d40b6b`进一步以root-owned安装树、`openat2` artifact descriptor和cgroup Commit后的`/proc/self/fd/N`执行关闭runtime executable替换窗口，Codex/root/bwrap三类替换攻击也各连续通过20轮。该组证据关闭满足安装合同的production Linux filesystem safe-open、process-tree containment与runtime safe-exec；其他平台、新release/architecture及真实Kubernetes装配仍须单独验收。
...
production gateway每次进程启动都生成新的gateway/producer identity，并在创建listener和发布readiness之前循环调用`POST /internal/v2/executor-connections/{executorId}:recover-gateway`。Core先用独立事务fence当前connection generation并把executor/environment置为offline；该事务一旦提交，`BeginOperationDispatch`、`AcknowledgeOperation`和`CompleteOperation`都会在实际状态变化前重新锁定并核对当前online connection、精确generation及数据库时钟lease，旧进程不能再提交迟到写入。随后Core在持有executor锁、阻止fresh generation出现的条件下，以每批最多64个execution执行第二个事务：`dispatching|acknowledged → unknown`，合法尾部`timeout_terminate prepared → skipped`，完全未发送且execution仍为`approved`的operation保持`prepared`；若所有operation已有可信终态则补交execution聚合终态。无法安全处理的其他必执行`prepared` operation使恢复事务整体fail closed，但已经提交的connection fence不回滚。

每个成功恢复的execution只写一条`execution.<aggregate>` canonical event/outbox，绑定恢复原因、被fence generation和operation变化。Core调用结果不明时同一gateway进程不得重试；进程必须退出，由新identity重新启动，已提交的terminal execution不会再次获得恢复事件。数据库事务用例、256-operation/64 KiB审计边界以及独立OS进程hard-kill矩阵已经覆盖Begin commit、不可逆发送、ACK、operation terminal和execution terminal前后；真实跨仓agentx/Kubernetes拓扑仍由Phase 5部署门禁负责。Kubernetes首版必须使用单副本和`Recreate`，不能配置HPA或用rolling overlap冒充高可用。
...
executor-gateway/core是产品审批的唯一策略权威。app-server默认以 Codex `permissionMode=read-only` 运行；session 的 mode 同时进入 gateway policy resolver：`read-only` 对 shell 保持 `ask`，`auto` 与 `full-access` 将受限 executor 工具映射为 `allow`，而部署基线 `deny` 永远优先。需要用户决定时，executor-gateway才在worker发起的原MCP `tools/call`上发送标准`elicitation/create`；harness-worker作为MCP client把该request与core approval record双向关联。approval record必须持久化绝对`expires_at`，core用CAS决定`approved|denied|expired|cancelled`，worker依据同一deadline设置本地兜底timer，不能让MCP elicitation或对应dynamic callback无限悬挂。联动 allow 只消除重复的人机确认，不绕过 backend sandbox、网络策略、capability、Core live-authority 或运行时安全检查。
...
A06旧probe证明stock app-server direct-MCP client能转接标准`elicitation/create`，但新架构刻意不走该路径。当前reference运行链已经接通gateway→worker MCP client→harness-control 1.3→pool/Core observe→canonical outcome→gateway consume：worker逐字段关联run/call/generation/catalog/execution/approval/nonce/context/version，只把`approved`映射为带九字段canonical evidence的`accept`，`denied|expired`映射为`decline`，`cancelled|consumed`及control关闭映射为`cancel`。`accept`本身仍不授权dispatch，只有gateway随后成功执行Core `ConsumeApproval`才可跨越发送边界。门禁已经覆盖ACK不等待用户决定、journal/replay、database-time expiry、nonce单次消费、stale generation、MCP/control取消和非accept零dispatch。开发stack把显式`maxApprovalTtl`签入run manifest，并以五run smoke覆盖approve、执行后cancel、deny、database-time expiry和pending时cancel；后三者必须同时证明execution无`dispatched_at`且无`execution_operations`。A06据此关闭。Phase 4的真实进程级transport fault gate现又分别覆盖pending approval期间的control mTLS断线/same-holder replay、MCP HTTP断线后迟到accept零dispatch，以及agentx WebSocket恢复窗口到期后的unknown收口；它们不由开发happy-path smoke代替。提交`cc02131`的四条精确gate已在OCI index digest `24c44fe44872962a828df84d3ff67ae2d541e076fad1e4101f9dc0dca5d8bf21`的pinned Linux arm64容器中各连续通过20次，Phase 4 transport-fault复核据此关闭。

A07 dynamic probe已经确认：pending `item/tool/call`时调用`turn/interrupt`会以`interrupted`结束turn，不发第二次模型请求，也永远不发`serverRequest/resolved`。正常dynamic response同样没有resolved。因此worker必须按类型清理：已回复call以JSON-RPC response写入完成为准，未回复call以所属turn terminal为准，并取消对应MCP request。reference `AppServerRunner`现已把bridge接入单reader、单writer的真实Codex wire事件循环：它固定`initialize → initialized → thread/start|resume → turn/start`参数面，以有界notification sink转发原消息，严格核对`thread/started`、`turn/started`与匹配的terminal，只允许`item/tool/call`反向请求；MCP result必须先claim再由同一writer写response，写成功后才释放，取消/bridge失败/缓冲溢出则先取消MCP、再`turn/interrupt`并有界等待terminal。stock在`interrupted|failed` terminal前可能只发送`item/started`而不补`item/completed`；pool adapter因此只为浏览器projection确定性闭合未完成message/reasoning/tool lifecycle，按item ID排序，并给tool输出无command presentation的“turn未完成”结果。它不能伪造executor成功或shell surface；`completed` terminal仍要求所有item原生闭合，否则fail closed。net.Pipe wire fixture与race门禁已经覆盖正常写屏障、terminal竞态、取消、未知request、写失败、事件溢出和resume catalog digest。新的非live组合门禁使用official SDK和真实HTTP session贯通runner→bridge→MCP client→gateway，并逐帧拒绝bearer进入app-server wire；强制断开in-flight MCP HTTP连接后，runner会interrupt、等到匹配terminal并清空callback。pending elicitation的独立断线门禁还会等worker decision context先被transport取消，再故意返回迟到`accept`，并证明gateway只收到一次call且dispatch为零。`MCPClient.Close`现在跟踪每个HTTP request，先等待可配置且有硬上限的graceful grace，再abort私有transport并有界返回，不会无限卡在SDK session DELETE。但已断的transport不可能把cancel送到已dispatch的远端handler；gateway必须自己执行connection grace、execution deadline和unknown/terminal收口，不能依赖worker的最后一帧。新的纵向门禁因此继续贯通official MCP client→真实executor MCP handler→`ShellExecutor`→真实agentx WebSocket：`process/start`已被agentx收到且Core operation进入`dispatching`后关闭socket，恢复窗口内execution保持原状态且没有ACK；测试只提前触发已经排定的生产30秒expiry callback，不改变协议窗口，随后验证start operation=`unknown`、未发送的timeout operation=`skipped`、execution=`unknown`、holder fenced且MCP只返回一次unknown结果。协议级A07缺口据此关闭；同一runner驱动stock app-server的live gate已在macOS arm64 stable binary SHA-256 `ae1d3ffe6d48aec6a4dc3f50e7eb8e0d11962485a6a9406c5a7012139383da02`、size `271056976`上通过，四条transport-fault精确测试也已在上述pinned Linux arm64环境各连续通过20次。
...
| D13 | executor-gateway/core 是唯一产品审批权威；gateway 读取同一签名 Codex `permissionMode` 计算 bounded executor tool 的 allow/ask（`read-only` 保留 ask，`auto`/`full-access` 联动 allow，显式 deny 优先），仅 ask 才向作为 MCP client 的 worker 发 elicitation；app-server 与 gateway 共享同一份冻结 mode authority | 让用户的单一 permission mode 同时约束 Codex 与 AgentServer，避免重复询问，同时保留 backend sandbox、网络、capability、RBAC 和 timeout/cancel/fence 语义 |
...
该状态仍不能称为生产可部署executor，但身份链、filesystem safe-open、Linux process-tree containment、runtime safe-exec以及gateway进程丢失后的数据库恢复审计已经闭合。Core的双钥enrollment、Hydra v26.2.0 ES256 `private_key_jwt`、opaque短期token在线授权/吊销已经实现；llmproxy和executor-gateway也已有production serve、本地Ed25519 run-capability验签及逐请求Core live-authorize。executor-gateway额外完成deployment-bound enrollment relay、256-bit/30秒进程内challenge、Ed25519 WSS proof、replay/expiry/Core故障/revoke门禁、TLS/mTLS SPIFFE装配和真实进程级production E2E；`serve --insecure-dev`只保留为显式开发入口。独立agentx的`548619a`身份/连接切片已实现stdin-only一次性enrollment、owner-only Ed25519/P-256双私钥与模糊提交恢复、detached-signed runtime release、RFC 7523 ES256换token、每次物理fresh/resume WSS的新challenge签名，以及connector→降权runner/exec-server的credential/FD/env隔离；`066acf6`以Linux `openat2`和只读继承descriptor关闭production filesystem path TOCTOU，`b4fb3eb`以CAP_CHOWN-only guardian、原子cgroup placement、迁移冻结事务和全退出`cgroup.kill`关闭`setsid`/double-fork逃逸，`5d40b6b`再以root-owned安装链、固定artifact descriptor与Commit后的pinned-inode exec关闭Codex/runtime-root/bwrap替换窗口。migration 0015、startup recovery API和hard-kill矩阵现已关闭`dispatching|acknowledged`恢复缺口；仍未完成的是生产Kubernetes/IAM/Secret/NetworkPolicy清单与全拓扑故障注入。任何未关联business RPC与不匹配MCP call metadata仍会fail closed。

进入实现前必须完成以下 Phase 0 gate：

- [ ] 固定Codex版本与Codex/外部executable digest，完成stock exec-server stdio启动/RPC fixture、outer capability排除`process/signal`、每process独占instance、正常关闭与root/descendant异常时无旁路process-tree回收、agentx代理兼容测试（reference adapter、真实agentx纵向兼容、Linux arm64 filesystem safe-open、cgroup v2 process-tree与`5d40b6b` runtime pinned-exec门禁已在exact stable 0.146.0通过；Linux amd64与非Linux平台仍不得沿用该native证据标production）。
...
- [ ] 完成 `PrepareExecution → approval → dispatching → ACK/running → terminal` 状态机，并在 DB commit、WSS send、agentx ACK、MCP response 各边界注入 crash，证明未知副作用不会自动重放（状态机、真实PostgreSQL recovery及独立Go进程按gateway transition顺序执行的hard-kill矩阵已完成；真实worker→gateway→agentx生产拓扑的MCP response边界仍待部署门禁）。
- [ ] 验证 executor 侧空 Codex home、清洗环境、无模型 credential、禁用 `--remote`/`http/request`，且不能回退到未校验系统 Codex。
- [ ] 完成 executor enrollment、独立机器身份、吊销、30 秒 WSS 恢复和 stdio child 跨重连存活测试（Core/Hydra/gateway与agentx两侧的enrollment、在线吊销、每连接challenge/proof/replay、同进程fresh/resume和child生命周期单侧门禁已完成；仍须在生产拓扑中执行跨仓、真实Hydra与gateway重连故障注入门禁）。
