...
func (s *Server) handleInternalLeaseScheduledTasks(w http.ResponseWriter, r *http.Request) {
	...
	// committed (rows stamped with lease_until + lease_owner, status still
	// 'pending' per C1 fix). If CreateScheduledTaskRun or last_run_id stamp
	// fails mid-batch we return 500; un-stamped tasks keep their lease until
	// it expires (leaseSeconds from now), at which point the next tick
	// re-claims them. Orphan run rows remain but are easy to garbage-collect
	// later (no FinalizeRunAndAdvance was ever called for them).
