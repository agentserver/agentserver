// Package braincatalog owns the versioned, deterministic model-visible tool
// catalog representation shared by core, harness-pool, and harness-worker.
...
// ParseCanonical validates a persisted catalog as the exact canonical output
// of this package. It rejects non-canonical encodings and unknown fields so a
// stored byte sequence cannot acquire new meaning after a decoder upgrade.
func ParseCanonical(raw []byte, limits Limits) (*Catalog, error) {
	if err := limits.Validate(); err != nil {
		return nil, err
	}
	value, canonical, err := DecodeCanonicalJSON(raw, limits.MaxCatalogBytes, limits)
	if err != nil {
		return nil, fmt.Errorf("decode canonical tool catalog: %w", err)
	}
	if !bytes.Equal(raw, canonical) {
		return nil, errors.New("tool catalog bytes are not RFC 8785 canonical JSON")
	}
	root, ok := value.(map[string]any)
	if !ok {
		return nil, errors.New("tool catalog root must be an object")
	}
	if err := requireExactObjectFields(root, "tool catalog", "canonicalizer", "namespace", "namespaceDescription", "tools"); err != nil {
		return nil, err
	}
	canonicalizer, ok := root["canonicalizer"].(string)
	if !ok || canonicalizer != CatalogCanonicalizer {
		return nil, fmt.Errorf("tool catalog canonicalizer must be %q", CatalogCanonicalizer)
	}
	namespace, ok := root["namespace"].(string)
	if !ok {
		return nil, errors.New("tool catalog namespace must be a string")
	}
	namespaceDescription, ok := root["namespaceDescription"].(string)
	if !ok {
		return nil, errors.New("tool catalog namespaceDescription must be a string")
	}
	rawTools, ok := root["tools"].([]any)
	if !ok {
		return nil, errors.New("tool catalog tools must be an array")
	}
	if len(rawTools) > limits.MaxTools {
		return nil, fmt.Errorf("tool catalog contains %d tools, limit is %d", len(rawTools), limits.MaxTools)
	}
	descriptors := make([]ToolDescriptor, len(rawTools))
	for index, rawTool := range rawTools {
		tool, ok := rawTool.(map[string]any)
		if !ok {
			return nil, fmt.Errorf("tool catalog tools[%d] must be an object", index)
		}
		if err := requireExactObjectFields(tool, fmt.Sprintf("tool catalog tools[%d]", index), "description", "inputSchema", "name"); err != nil {
			return nil, err
		}
		name, ok := tool["name"].(string)
		if !ok {
			return nil, fmt.Errorf("tool catalog tools[%d].name must be a string", index)
		}
		description, ok := tool["description"].(string)
		if !ok {
			return nil, fmt.Errorf("tool catalog tools[%d].description must be a string", index)
		}
		descriptors[index] = ToolDescriptor{Name: name, Description: description, InputSchema: tool["inputSchema"]}
	}

	catalog, err := BuildCatalog(namespace, namespaceDescription, descriptors, limits)
	if err != nil {
		return nil, err
	}
	if !bytes.Equal(catalog.canonical, raw) {
		return nil, errors.New("tool catalog is not in deterministic name order")
	}
	return catalog, nil
}

func requireExactObjectFields(object map[string]any, label string, fields ...string) error {
	if len(object) != len(fields) {
		return fmt.Errorf("%s must contain exactly fields %s", label, strings.Join(fields, ", "))
	}
	for _, field := range fields {
		if _, ok := object[field]; !ok {
			return fmt.Errorf("%s is missing field %q", label, field)
		}
	}
	return nil
}
...
func (c *Catalog) DigestSHA256() [sha256.Size]byte {
	var digest [sha256.Size]byte
	decoded, err := hex.DecodeString(c.digest)
	if err != nil || len(decoded) != len(digest) {
		panic("braincatalog: internally generated digest is invalid")
	}
	copy(digest[:], decoded)
	return digest
}
...
func (c *Catalog) Limits() Limits { return c.limits }
...
// ValidateNamespace validates the exact dynamic-tool namespace grammar used
// by both catalog construction and worker configuration.
func ValidateNamespace(name string, maxBytes int) error {
	return validateNamespace(name, maxBytes)
}
...
// ValidateNameText validates a required, bounded protocol identifier.
func ValidateNameText(label, value string, maxBytes int) error {
	return validateNameText(label, value, maxBytes)
}
...
// ValidateText validates bounded UTF-8 protocol text without NUL.
func ValidateText(label, value string, maxBytes int) error {
	return validateText(label, value, maxBytes)
}
