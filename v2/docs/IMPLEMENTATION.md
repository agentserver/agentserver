...
production-shape probe验证了stock `thread/start.dynamicTools` client bridge。stable 0.146.0与0.147.0-alpha.2在完全不配置Codex MCP server时，实际模型工具面都精确只有`executor.approved_echo`；真实namespaced function call变成带原thread/turn/call id、tool和结构化参数的`item/tool/call`，client回包结果进入下一次模型请求。模型伪造未公布executor tool或`exec_command`都只得到`unsupported call`，不会产生callback。pending dynamic call可由`turn/interrupt`结束，但正常回包和interrupted terminal都不产生`serverRequest/resolved`。A03、A04、A05与A09已据此改门禁并通过；A06/A07/A08/A11/A12按本文列出的新边界继续实施。
...
1. 具体stock Codex release/tag：stable 0.146.0是当前修订门禁candidate，只有A06–A08、A11/A12、E03/E07 adapter及目标平台E09全部通过后才写production manifest。
