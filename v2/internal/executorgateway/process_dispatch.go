...
// DispatchProcess journals and writes one deterministic process/start frame.
...
func (s *Server) DispatchProcess(ctx context.Context, request ProcessDispatchRequest) (*ProcessExchange, error) {
	...
	runtime, connection, holder, err := s.readyProcessRuntime(request.ExecutorID, request.Context.EnvID)
	if err != nil {
		return nil, err
	}
...
func (s *Server) readyProcessRuntime(executorID, environmentID string) (*sessionRuntime, *websocket.Conn, ConnectionHolder, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
	if s.shuttingDown {
...
// ProcessExchange retains the matching process/start response and the exact
// ordered process notifications until process/closed. It is scoped to one
// connection generation and one immutable routing context.
...
func (table *processCallTable) register(holder ConnectionHolder, request ProcessDispatchRequest) (*ProcessExchange, error) {
	...
	if message.Kind != codexwire.KindRequest || message.Method != execprofile.MethodProcessStart {
		return nil, errors.New("process dispatch accepts exactly one process/start request")
	}
	...
	if err := message.DecodeParams(&params); err != nil || params.ProcessID == "" {
		return nil, errors.New("process/start requires processId")
	}
...
func matchProcessCall(call *processCall, holder ConnectionHolder, routing *agentxconn.RoutingContext) error {
	...
	if routing == nil || *routing != call.routing {
